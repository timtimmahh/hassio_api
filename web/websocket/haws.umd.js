(function (global, factory) {
  ...
  function createSocket(options) {
    ...
    function connect(triesLeft, promResolve, promReject) {
      ...
      const handleOpen = async (event) => {
        try {
          if (auth$1.expired) {
            await (authRefreshTask ? authRefreshTask
                : auth$1.refreshAccessToken());
          }
    ...
    return new Promise(
        (resolve, reject) => connect(options.setupRetry, resolve, reject));
  }
  ...
  class Connection {
    ...
    fireEvent(eventType, eventData) {
      (this.eventListeners.get(eventType) || []).forEach(
          (callback) => callback(this, eventData));
    }
    ...
    async subscribeMessage(callback, subscribeMessage, options) {
      ...
      await new Promise((resolve, reject) => {
        ...
        info = {
          ...
          reject,
          callback,
          subscribe: (options === null || options === void 0 ? void 0
  ...
  function redirectAuthorize(hassUrl, clientId, redirectUrl, state) {
    ...
    document.location.href = genAuthorizeUrl(hassUrl, clientId, redirectUrl,
        state);
  }
  ...
  async function getAuth(options = {}) {
    ...
    const clientId = options.clientId !== undefined ? options.clientId
        : genClientId();
    ...
    redirectAuthorize(hassUrl, clientId,
        options.redirectUrl || genRedirectUrl(), encodeOAuthState({
          hassUrl,
          clientId,
        }));
  ...
  const getCollection = (conn, key, fetchCollection, subscribeUpdates) => {
    ...
    const refresh = () => fetchCollection(conn).then(
        (state) => store.setState(state, true));
  ...
  const createCollection = (key, fetchCollection, subscribeUpdates, conn,
      onChange) => getCollection(conn, key, fetchCollection,
      subscribeUpdates).subscribe(onChange);
  ...
  const callService = (connection, domain, service, serviceData,
      target) => connection.sendMessagePromise(
      callService$1(domain, service, serviceData, target));
  ...
  const subscribeUpdates$2 = (conn, store) => Promise.all([
    conn.subscribeEvents(store.action(processComponentLoaded),
        "component_loaded"),
    conn.subscribeEvents(
        () => fetchConfig(conn).then((config) => store.setState(config, true)),
        "core_config_updated"),
  ]).then((unsubs) => () => unsubs.forEach((unsub) => unsub()));
  const configColl = (conn) => getCollection(conn, "_cnf", fetchConfig,
      subscribeUpdates$2);
  const subscribeConfig = (conn, onChange) => configColl(conn).subscribe(
      onChange);
  ...
  function processServiceRegistered(conn, store, event) {
    ...
    if (!((_a = state.domain) === null || _a === void 0 ? void 0
        : _a.service)) {
  ...
  const debouncedFetchServices = debounce(
      (conn, store) => fetchServices(conn).then(
          (services) => store.setState(services, true)), 5000);
  const fetchServices = (conn) => getServices(conn);
  const subscribeUpdates$1 = (conn, store) => Promise.all([
    conn.subscribeEvents((ev) => processServiceRegistered(conn, store, ev),
        "service_registered"),
    conn.subscribeEvents(store.action(processServiceRemoved),
        "service_removed"),
  ]).then((unsubs) => () => unsubs.forEach((fn) => fn()));
  const servicesColl = (conn) => getCollection(conn, "_srv", fetchServices,
      subscribeUpdates$1);
  const subscribeServices = (conn, onChange) => servicesColl(conn).subscribe(
      onChange);
  ...
  const subscribeUpdates = (conn, store) => conn.subscribeEvents(
      (ev) => processEvent(store, ev), "state_changed");
  const entitiesColl = (conn) => getCollection(conn, "_ent", fetchEntities,
      subscribeUpdates);
  const subscribeEntities = (conn, onChange) => entitiesColl(conn).subscribe(
      onChange);
